...
linguagem Go, e seguimos o mesmo modelo  do JavaSpaces para a representação 
...
modo simultâneo e eficientemente. Para garantir isso, a implementação proposta levou
em consideração as seguintes decisões arquiteturais.
...
\begin{figure}[h]
	\center
	\includegraphics[width=.60\textwidth]{img/indice_invertido.png}
...
\begin{figure}[h]
	\center
	\includegraphics[width=.75\textwidth]{img/indice_tuplas.png}
...
muitas escritas são realizadas, ao custo de operações de leitura rápidas.
...
Porém, quando uma tupla é encontrada, esta é removida do índice central, que associa
o identificador aos dados utilizando uma tabela hash padrão da linguagem Go.
...
contra cada requisição na fila de espera para evitar que uma nova requisição tome a
frente de uma que esteja a espera. Essa busca na fila de espera é necessária para
que requisições do tipo \emph{take()} sejam tratadas corretamente.
...
rudimentar percorre os índices em períodos regulares em busca de tuplas que tenham expirado o \emph{leasing}. 
...
\begin{figure}[h]
	\center
	\includegraphics[width=.85\textwidth]{img/arquitetura.png}
...
%TODO: Falar das máquinas utilizadas (Azure, etc)?
...
Para cada número de requisições, foram executados casos de teste diferentes com o servidor utilizando 1, 2, 4 e 8 gorotinas.
...
\begin{figure}[h]
	\center
	\includegraphics[width=.8\textwidth]{img/performance2.png}
...
\begin{figure}[h]
	\center
	\includegraphics[width=.8\textwidth]{img/performance1.png}
...
Porém, o aumento do número de requisições no Espaço de Tuplas, e o consequente aumento na quantidade de escrita de tuplas degrada consideravelmente o sistema, mesmo com o maior número de servidores testado.
